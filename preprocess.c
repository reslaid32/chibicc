...
static Hideset*
hideset_union(Hideset* hs1, Hideset* hs2)
{
  Hideset head = {};
  Hideset* cur = &head;
...
static bool
hideset_contains(Hideset* hs, char* s, int len)
{
  for (; hs; hs = hs->next)
...
static Hideset*
hideset_intersection(Hideset* hs1, Hideset* hs2)
{
  Hideset head = {};
  Hideset* cur = &head;
...
static Token*
add_hideset(Token* tok, Hideset* hs)
{
  Token head = {};
  Token* cur = &head;
...
static Token*
new_str_token(char* str, Token* tmpl)
{
...
static Token*
copy_line(Token** rest, Token* tok)
{
  Token head = {};
  Token* cur = &head;
...
static Token*
new_num_token(int val, Token* tmpl)
{
  char* buf = format("%d\n", val);
  return tokenize(new_file(tmpl->file->name, tmpl->file->file_no, buf));
}
...
static Token*
read_const_expr(Token** rest, Token* tok)
{
...
static long
eval_const_expr(Token** rest, Token* tok)
{
  Token* start = tok;
...
static CondIncl*
push_cond_incl(Token* tok, bool included)
{
...
static Macro*
add_macro(char* name, bool is_objlike, Token* body)
{
...
static MacroParam*
read_macro_params(Token** rest, Token* tok, char** va_args_name)
{
  MacroParam head = {};
  MacroParam* cur = &head;
...
static void
read_macro_definition(Token** rest, Token* tok)
{
  if (tok->kind != TK_IDENT)
...
static MacroArg*
read_macro_arg_one(Token** rest, Token* tok, bool read_rest)
{
  Token head = {};
  Token* cur = &head;
  int level = 0;
...
static MacroArg*
find_arg(MacroArg* args, Token* tok)
{
...
static char*
join_tokens(Token* tok, Token* end)
{
...
static Token*
stringize(Token* hash, Token* arg)
{
...
static bool
expand_macro(Token** rest, Token* tok)
{
...
char*
search_include_paths(char* filename)
{
  if (filename[0] == '/')
    return filename;
...
static char*
search_include_next(char* filename)
{
...
static char*
read_include_filename(Token** rest, Token* tok, bool* is_dquote)
{
...
//   #endif
static char*
detect_include_guard(Token* tok)
{
...
static Token*
include_file(Token* tok, char* path, Token* filename_tok)
{
...
// Read #line arguments
static void
read_line_marker(Token** rest, Token* tok)
{
  Token* start = tok;
...
void
define_macro(char* name, char* buf)
{
...
static Macro*
add_builtin(char* name, macro_handler_fn* fn)
{
...
// as per the C spec.
static void
join_adjacent_string_literals(Token* tok)
{
